...
export default async function StatementPage({
  ...
  const [monthlyR, scalarsR, expensesR] = await Promise.all([
  ...
  // Financial position (from recorded data)
  const cashCollectedLifetime = Number(s.total_collected ?? 0);
  const receivables = Number(s.owed_total ?? 0);
  ...
  return (
    <div className="mx-auto max-w-3xl print:max-w-none">
      ...
      <div className="glass-card rounded-3xl p-8 print:rounded-none print:border-none print:bg-white print:shadow-none">
        ...
        {/* Financial position */}
        <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-muted">Financial Position (as of {MONTHS[cm]} {year})</p>
        <div className="text-sm">
          <Row label="Cash collected to date" value={formatNaira(cashCollectedLifetime)} />
          <Row label="Owed to you (receivables)" value={formatNaira(receivables)} />
          <Row label="Total position" value={formatNaira(cashCollectedLifetime + receivables)} bold />
        </div>
